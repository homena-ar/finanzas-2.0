  ...
  try {
    ...
    if (!to || !userName || !resetLink) {
      return NextResponse.json(
        { error: 'Faltan campos requeridos: to, userName, resetLink' },
        { status: 400 }
      )
    }
    ...
    const emailTemplate = getPasswordResetTemplate(userName, resetLink)
    ...
    if (result.error) {
      console.error('❌ [API] Error enviando correo de recuperación:', result.error)
      return NextResponse.json(
    ...
    console.error('❌ [API] Error en send-password-reset-email:', error)
    return NextResponse.json(
